%DataVisu;
% Scipt DataVisu to be called or copy & past within the picgui script 
Tserial = 5e-4;
if ~ishandle(50) figure(50); end % new fig
set(0,'CurrentFigure',50);  % select fig
[A, tUart] = picgui.padr(Rn,6,t_Rn);  % sample on last column data received (remove NaN numbers from the original matrix)
% considere signed values (all channels)
idx = find(A > 65535 / 2);
A(idx)= A(idx)-65536;
    
if ~isempty(A)
    clf 
    t = ([0:(length(A)-1)]  + tUart(1))*Tserial; % Do not rely on UART data arrival timestamp. Recreate time vector
    subplot(3,1,1); plot(t,0.0012*A(:,1:2)); axis tight; ylabel('v_d_q (in V)'); ylim([-20 20]);    
    subplot(3,1,2); plot(t,6.6620e-04*A(:,3:4)); axis tight; ylabel('i_d_q (in A)'); ylim([-10 10]);
    subplot(3,1,3); plot(t,A(:,5),t,2^-6*A(:,6)); axis tight; ylabel('om, omr (in rad/s)'); ylim([-500 500]);
    xlabel('time (in s)')


    % % dim=[0 .9 .1 .1] ;
    % % if(A(end,1)==0)
    % % annotation('textbox',dim,'String',sprintf('Mode = Voltage'));
    % % elseif(A(end,1)==1)
    % % annotation('textbox',dim,'String',sprintf('Mode = Speed'));
    % % end
    % % 
    % 
    % dim=[0.25 .9 .1 .1] ;
    % str = 'vDC = %.2f V';
    % annotation('textbox',dim,'String',sprintf(str,0.001057*A(end,9)));
    % dim=[0.5 .9 .1 .1] ;
    % str = 'vMAX = %.2f V';
    % annotation('textbox',dim,'String',sprintf(str,0.0012*A(end,8)));
    % dim=[0.75 .9 .1 .1] ;
    % str = 'CPU = %.2f %%';
    % annotation('textbox',dim,'String',sprintf(str,2^-8*A(end,10)));
end
 