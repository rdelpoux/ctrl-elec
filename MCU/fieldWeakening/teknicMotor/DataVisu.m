...
if ~isempty(A)
    ...
    subplot(3,1,2); plot(t,21.83/32768*A(:,[4 5])); axis tight; ylabel('ird irq [A])'); ylim([-10 10]);
    hold on;  plot(t,2^-11*A(:,[7 8])); axis tight; ylabel('id iq  [A]');; 
    hold off  ; ylim([-10 10])
    subplot(3,1,3); plot(t,[A(:,9:10)]); axis tight; ylabel('om, omr [rad/s])');  ylim([0 700]);
